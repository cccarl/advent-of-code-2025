...
pub fn day2(mut input_reader: BufReader<File>) {
    ...
    loop {
        ...
        let range_start: u64 = range_split
            .next()
            .unwrap_or_else(|| panic!("Could not get range start in line: {}", range_str))
            .to_string()
            .parse()
            .unwrap_or_else(|_| {
                panic!("Could not parse to int range start in line: {}", range_str)
            });
        let range_end: u64 = range_split
            .next()
            .unwrap_or_else(|| panic!("Could not get range end in line: {}", range_str))
            .to_string()
            .parse()
            .unwrap_or_else(|_| {
                panic!("Could not parse to int range start in line: {}", range_str)
